...
- **Config**: JSONC format (`opencode-mmcp-1file.jsonc`), loaded via `loadConfig()` with 15 sections (chatMessage, autoCapture, compaction, keywordDetection, preemptiveCompaction, privacy, compactionSummaryCapture, codeIndexSync, preferenceLearning, learningMemory, captureModel, memoryScope, mcpServer, systemPrompt, performance). `preferenceLearning` is a legacy/compatibility alias for `learningMemory`. `chatMessage` and `compaction` carry bootstrap limits/budgets; `performance` carries recall/project/learning plus bootstrap, observation, audit, and search-trace timeouts; `mcpServer` also carries HTTP reconnect/heartbeat timing.
