...
- **Config**: JSONC format (`opencode-mmcp-1file.jsonc`), loaded via `loadConfig()` with 12 sections (chatMessage, autoCapture, compaction, keywordDetection, preemptiveCompaction, privacy, compactionSummaryCapture, codeIndexSync, captureModel, memoryScope, mcpServer, systemPrompt). `mcpServer` also carries HTTP reconnect/heartbeat timing.
