...
10. **Performance timeouts** (ask only if the user has a large database or reports slow memory retrieval):
    - "Do you have a large memory database (>10k memories) or experience slow context injection? You can increase per-call MCP timeouts."
    - Default timeouts: recall/project knowledge = 15s, others = 10s
    - Cache TTL: `projectInfoCacheTtlMs` controls how long `project_info` data is cached in memory (default: 5 minutes / 300000ms)
    - This maps to the `performance` section
...
```jsonc
{
  ...
  // Increase timeout values for large databases (>10k memories).
  "performance": {
