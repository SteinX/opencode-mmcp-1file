...
const plugin: Plugin = async (input) => {
  ...
  const ensureRetryLoop = () => {
    ...
    startRetryLoop(() => tryReconnect(config), config.mcpServer.reconnectIntervalMs, handleReconnectSuccess, connectionKey)
  }
  ...
  const cleanup = async () => {
    ...
    stopRetryLoop(connectionKey)
    retryLoopStarted = false
    registerConnectionFailureHandler(null, connectionKey)
  ...
  return {
    "chat.message": async (hookInput, output) => {
      ...
      if (userText) {
        if (config.learningMemory?.enabled) {
          void learnFromChatMessage(config, userText, { source: "chat.message" }).catch((err: unknown) =>
            logger.debug("Background learn failed", { error: String(err) })
          )
        } else {
          void learnPreferencesFromText(config, userText, {
            source: "chat.message",
          }).catch((err: unknown) =>
            logger.debug("Background learn failed", { error: String(err) })
          )
      ...
      const learnedPreferencesPromise: Promise<string | null> = withTimeout((() => {
      ...
      })(), config.performance?.learningMemoryTimeoutMs ?? 10_000, { fallback: null, label: "learning-memory" })
      ...
      for (const r of results) {
        if (r.status === "rejected") {
          logger.warn("Context source failed during injection", { error: String(r.reason) })
        }
      }
    ...
    event: async (eventInput) => {
      ...
      if (event.type === "session.compacted" && config.compaction.enabled) {
        const sessionID = event.properties?.sessionID
        if (!sessionID) return
        const compactSummary = extractCompactSummary(event)
        ...
        await handleCompactionRecovery(config, input, sessionID, compactSummary)
      }
    ...
    "experimental.chat.system.transform": async (_hookInput, output) => {
      ...
      const promptText = buildMemorySystemPrompt(config, availablePluginTools, !isConnectionFailed(connectionKey))
    ...
    "experimental.session.compacting": async (hookInput, output) => {
      ...
      const recovery = await buildCompactionRecoveryContext(config, extractCompactSummary(hookInput))
      if (recovery) {
...
function extractCompactSummary(input: any): string | undefined {
  const candidates = [
    input?.summary,
    input?.compactSummary,
    input?.context?.summary,
    input?.properties?.summary,
    input?.properties?.compactSummary,
    input?.properties?.info?.summary,
    input?.event?.summary,
    input?.event?.properties?.summary,
    input?.event?.properties?.compactSummary,
    input?.event?.properties?.info?.summary,
  ]
...
async function handleCompactionRecovery(
  ...
  try {
    const recovery = await buildCompactionRecoveryContext(config, compactSummary)
    if (!recovery) return
