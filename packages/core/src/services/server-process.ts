...
function getDataDirLabel(config: PluginConfig): string {
  return resolveDataDir(config) ?? "(unresolved dataDir)"
}
...
function assertLockMatchesConfig(config: PluginConfig, lock: LockFileData, url: string): void {
  if (lock.port === config.mcpServer.port && lock.bind === config.mcpServer.bind) return

  throw new Error(
    `HTTP MCP server lock for ${getDataDirLabel(config)} points to ${lock.bind}:${lock.port}, `
      + `but requested ${url}. Use the same dataDir/tag and endpoint to share the server, `
      + "or configure a different mcpServer.port.",
  )
}

function joinExistingLockedServer(config: PluginConfig, lockPath: string, lock: LockFileData, url: string, myPid: number): string {
  assertLockMatchesConfig(config, lock, url)
...
function throwPortOwnedByDifferentDataDir(config: PluginConfig, url: string): never {
  throw new Error(
    `HTTP MCP server is already healthy at ${url}, but no matching lock file exists for ${getDataDirLabel(config)}. `
      + "Use the same mcpServer.dataDir/tag to share it, or configure a different mcpServer.port.",
  )
}
...
async function ensureServerRunningUnshared(config: PluginConfig): Promise<string> {
  ...
  if (await isServerRunning(config)) {
    const lock = readLockFile(lockPath)
    if (lock) {
      return joinExistingLockedServer(config, lockPath, lock, url, myPid)
    }
    throwPortOwnedByDifferentDataDir(config, url)
  }
  ...
  while (!startupOwner) {
    ...
    if (!lock || isStartupLockStale(lock) || !isProcessAlive(lock.ownerPid)) {
      ...
      if (await isServerRunning(config)) {
        ...
        if (existingLock) {
          return joinExistingLockedServer(config, lockPath, existingLock, url, myPid)
        }
