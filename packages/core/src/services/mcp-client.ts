...
interface ClientState {
  key: string
  client: Client | null
  connectionPromise: Promise<Client> | null
  resetConnectionPromise: Promise<Client> | null
...
function getClientKey(config: PluginConfig): string {
...
export function getMemoryConnectionKey(config: PluginConfig): string {
  return getClientKey(config)
}

function getClientState(config: PluginConfig): ClientState {
...
async function disposeClient(state: ClientState): Promise<void> {
  stopHeartbeat(state)
...
function startHeartbeat(config: PluginConfig, state: ClientState): void {
  stopHeartbeat(state)

  if (config.mcpServer.transport !== "http" || !state.client) return

  state.heartbeatTimer = setInterval(() => {
    if (!state.client || state.heartbeatPromise) return

    state.heartbeatPromise = (async () => {
      const healthy = await getCachedClientHealth(config, state)
      if (healthy || !state.client) return
...
async function getCachedClientHealth(config: PluginConfig, state: ClientState): Promise<boolean> {
  if (config.mcpServer.transport !== "http") return true

  const now = Date.now()
  if (state.healthCheckPromise) return state.healthCheckPromise
  if (now - state.lastHealthCheckAt < HEALTH_CHECK_THROTTLE_MS) return true
...
async function invalidateUnhealthyCachedClient(config: PluginConfig, state: ClientState): Promise<void> {
  const healthy = await getCachedClientHealth(config, state)
  if (healthy || !state.client) return
...
function notifyConnectionFailure(state: ClientState): void {
...
async function resetClientConnection(config: PluginConfig): Promise<Client> {
  ...
  if (state.resetConnectionPromise) return state.resetConnectionPromise
...
export function registerConnectionFailureHandler(handler: (() => void) | null, key?: string): void {
...
export async function resetMemoryClientForServerControl(config?: PluginConfig): Promise<void> {
  if (config) {
    const key = getClientKey(config)
    const state = clientStates.get(key)
    if (!state) return
