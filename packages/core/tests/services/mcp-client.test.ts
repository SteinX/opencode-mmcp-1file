...
function makeConfig(transportOverride?: "stdio" | "http"): PluginConfig {
  return {
    ...
    mcpServer: { command: ["npx", "-y", "@steinx/memory-mcp-1file"], tag: "default", model: "qwen3", transport: transportOverride ?? "stdio", port: 23817, bind: "127.0.0.1", reconnectIntervalMs: 30000, heartbeatIntervalMs: 20000, mcpServerName: "memory-mcp-1file" },
...
function createMockConnectionState() {
  ...
  return {
    isConnectionFailed: vi.fn((key?: string) => failedKeys.has(key ?? "default")),
    markConnectionFailed: vi.fn((key?: string) => {
      failedKeys.add(key ?? "default")
    }),
    markConnectionHealthy: vi.fn((key?: string) => {
      failedKeys.delete(key ?? "default")
    }),
  }
}
...
async function setupModule() {
  ...
  mockConnectionState = createMockConnectionState()
...
async function setupModuleWithClientFactory(
  clients: ReturnType<typeof createMockClient>[],
  resolveDataDirMock: (config: PluginConfig) => string = () => "/tmp/test-data",
) {
  vi.resetModules()
  ...
  vi.doMock("../../src/config.js", async (importOriginal) => {
    const original = await importOriginal<typeof import("../../src/config.js")>()
    return { ...original, resolveDataDir: vi.fn(resolveDataDirMock) }
  })
  vi.doMock("../../src/services/connection-state.js", () => mockConnectionState)
...
describe("mcp-client", () => {
  ...
  it("getMemoryClient reuses existing connection for the same config key", async () => {
    const { mod, mockClient } = await setupModule()
    const config = makeConfig()
  ...
  it("getMemoryClient keeps separate clients for different workspace configs", async () => {
    const firstClient = createMockClient()
    const secondClient = createMockClient()
    const { mod, mockStdioTransport } = await setupModuleWithClientFactory(
      [firstClient, secondClient],
      (config) => `/tmp/${config.mcpServer.tag}`,
    )
    const firstConfig = makeConfig("stdio")
    firstConfig.mcpServer.tag = "workspace-a"
    const secondConfig = makeConfig("stdio")
    secondConfig.mcpServer.tag = "workspace-b"

    const client1 = await mod.getMemoryClient(firstConfig)
    const client2 = await mod.getMemoryClient(secondConfig)
  ...
  it("connection failure for one workspace config does not block another config", async () => {
    const failingClient = createMockClient()
    const healthyClient = createMockClient()
    failingClient.connect.mockRejectedValueOnce(new Error("workspace-a failed"))
    const { mod } = await setupModuleWithClientFactory(
    ...
    await expect(mod.getMemoryClient(firstConfig)).rejects.toThrow("workspace-a failed")
    await expect(mod.getMemoryClient(secondConfig)).resolves.toBe(healthyClient)

    expect(mockConnectionState.isConnectionFailed).toHaveBeenCalledWith(
      expect.stringContaining("workspace-a"),
    )
    expect(mockConnectionState.isConnectionFailed).toHaveBeenCalledWith(
      expect.stringContaining("workspace-b"),
    )
    expect(mockConnectionState.markConnectionFailed).toHaveBeenCalledWith(
      expect.stringContaining("workspace-a"),
    )
    expect(healthyClient.connect).toHaveBeenCalledTimes(1)
  })
