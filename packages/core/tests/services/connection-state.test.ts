...
describe("connection-state", () => {
  ...
  describe("isConnectionFailed", () => {
    ...
    it("keeps failed state isolated by key", () => {
      markConnectionFailed("workspace-a")

      expect(isConnectionFailed("workspace-a")).toBe(true)
      expect(isConnectionFailed("workspace-b")).toBe(false)
      expect(isConnectionFailed()).toBe(false)
    })
  })
  ...
  describe("getConnectionStatus", () => {
    ...
    it("returns status for the requested key", () => {
      markConnectionFailed("workspace-a")

      expect(getConnectionStatus("workspace-a")).toMatchObject({
        connected: false,
        failureCount: 1,
      })
      expect(getConnectionStatus("workspace-b")).toMatchObject({
        connected: true,
        failureCount: 0,
      })
    })
  ...
  describe("startRetryLoop", () => {
    ...
    it("keeps retry loops isolated by key", async () => {
      const retryFn1 = vi.fn().mockResolvedValue(false)
      const retryFn2 = vi.fn().mockResolvedValue(false)
      startRetryLoop(retryFn1, 5_000, undefined, "workspace-a")
      startRetryLoop(retryFn2, 5_000, undefined, "workspace-b")

      await vi.advanceTimersByTimeAsync(5_000)

      expect(retryFn1).toHaveBeenCalledTimes(1)
      expect(retryFn2).toHaveBeenCalledTimes(1)
      expect(getConnectionStatus("workspace-a").retrying).toBe(true)
      expect(getConnectionStatus("workspace-b").retrying).toBe(true)
    })
  })
