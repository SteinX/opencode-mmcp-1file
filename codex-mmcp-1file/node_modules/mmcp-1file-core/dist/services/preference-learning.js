...
const preferenceStoreDebounceByScope = new Map();
const preferenceInjectedSessions = new Set();
...
function normalizePreferenceContent(text) {
    return text.trim().replace(/\s+/g, " ").toLowerCase();
}
function formatStoredPreferenceContent(content) {
    return `${PREFERENCE_PREFIX} ${content.trim()}`;
}
function parseStoredPreferenceContent(content) {
    const trimmed = content.trim();
    if (!trimmed.startsWith(PREFERENCE_PREFIX))
        return null;
    const raw = trimmed.slice(PREFERENCE_PREFIX.length).trim();
    return raw || null;
}
function applyPerCallPreferenceLimits(candidates, options) {
    const confirmed = [];
    const tentative = [];
...
function buildPreferenceScopeContext(config, context) {
...
function buildPreferenceScopeKey(config, context) {
...
function buildCandidateMetadata(config, candidate, context) {
    return {
        ...(context?.metadata ?? {}),
...
export function resetPreferenceLearningStoreStateForTests() {
    preferenceStoreDebounceByScope.clear();
    preferenceInjectedSessions.clear();
}
function normalizePreferenceStatus(value) {
    if (value === "confirmed" || value === "candidate")
        return value;
    return null;
}
function extractPreferenceStatus(memory) {
    const metadata = memory.metadata;
    if (metadata && typeof metadata === "object" && !Array.isArray(metadata)) {
        const status = normalizePreferenceStatus(metadata.status);
        if (status)
            return status;
    }
    return "confirmed";
}
function formatLearnedPreferenceSection(title, hint, values) {
    if (values.length === 0)
        return null;
    return [
        `## ${title}`,
...
export function markLearnedPreferencesInjected(sessionID) {
    preferenceInjectedSessions.add(sessionID);
}
export function markLearnedPreferencesCompacted(sessionID) {
    preferenceInjectedSessions.delete(sessionID);
}
export async function fetchAndFormatLearnedPreferences(config, context, dependencies) {
    if (!config.preferenceLearning.enabled)
        return null;
    const listFn = dependencies?.listFn ?? listProjectMemories;
    const scopeContext = buildPreferenceScopeContext(config, context);
    try {
        const result = await listFn(config, Math.max(1, config.preferenceLearning.maxStoredPreferences), false, scopeContext);
        if (result.status !== "ok")
            return null;
        const confirmed = [];
        const candidates = [];
        ...
        for (const memory of result.memories) {
            ...
            if (confirmed.length >= config.preferenceLearning.maxPreferences
        ...
        const sections = [
            ...
            formatLearnedPreferenceSection("Confirmed Preferences", "Treat these as strong guidance from prior user signals:", confirmed),
            formatLearnedPreferenceSection("Candidate Preferences", "Tentative hints only — validate before treating as hard rules:", candidates),
        ].filter(Boolean);
...
function firstMatchExcerpt(text, patterns) {
...
export function detectPreferenceSignal(text, options, context) {
    ...
    if (!signalType)
        return null;
    const excerptPatterns = signalType === "correction"
        ? CORRECTION_PATTERNS
        : signalType === "negation"
            ? NEGATION_PATTERNS
            : EXPLICIT_PATTERNS;
...
export function buildPreferenceExtractionPrompt(text, signal) {
    const systemPrompt = [
...
function normalizeRawCandidate(raw) {
...
export function parsePreferenceCandidates(rawResponse, signalType, options) {
    ...
    for (const item of parsed) {
        const normalized = normalizeRawCandidate(item);
        if (!normalized)
            continue;
        const confidence = normalized.confidence;
        if (confidence < options.candidateConfidence)
            continue;
        const status = confidence >= options.minConfidence ? "confirmed" : "candidate";
        candidates.push({
...
export async function extractPreferenceCandidates(config, text, signal, llmCaller = callChatCompletion) {
...
export async function storePreferenceCandidates(config, candidates, context, dependencies) {
    const result = {
        stored: 0,
        skippedDuplicate: 0,
        skippedPrivate: 0,
    ...
    const existingResult = await listFn(config, Math.max(1, config.preferenceLearning.maxStoredPreferences), false, scopeContext);
    const existingPreferenceSet = new Set();
