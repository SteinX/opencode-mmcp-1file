...
function compactionContext(config) {
...
export async function buildCompactionRecoveryContext(config, compactSummary) {
    ...
    const [taskResult, contextResult] = await Promise.all([
        ...
        recallMemories(config, "recent project context and decisions", config.compaction.memoryLimit, context),
    ]);
    ...
    if (bootstrapDiagnostics)
        parts.push(bootstrapDiagnostics);
    return {
        text: parts.join("\n\n"),
        count: totalCount,
