...
function getLearningMetadata(record) {
...
function getSourceLabel(record) {
    ...
    if (createdFrom)
        labels.push(createdFrom);
    const client = asString(source?.client);
    if (client)
        labels.push(client);
    const sourceIds = Array.isArray(source?.source_memory_ids)
        ? source?.source_memory_ids.filter((value) => typeof value === "string" && value.length > 0)
        : [];
    if (sourceIds.length > 0)
        labels.push(`ids:${sourceIds.join(",")}`);
    return labels.length > 0 ? labels.join(" / ") : "unknown";
}
function getEvidence(record) {
    const learning = getLearningMetadata(record);
...
function classifySection(record) {
    const kind = getRecordKind(record);
    const status = getRecordStatus(record);
    if (status === "candidate")
        return "candidate_signals";
    if (kind === "workflow_rule" || status === "rule")
        return "hard_rules";
    if (kind === "user_preference" && status === "confirmed")
        return "confirmed_user_preferences";
    if (kind === "project_lesson")
        return "relevant_project_lessons";
    if (kind === "project_pattern" || kind === "project_pitfall")
        return "relevant_patterns_pitfalls";
    return null;
}
function prepareRecord(record, includeEvidence) {
    return {
...
function formatSection(title, hint, records) {
    if (records.length === 0)
        return null;
    return [
        `## ${title}`,
...
export function formatLearningMemoryInjection(input, config) {
    ...
    for (const record of input.records) {
        ...
        if (!sectionId)
            continue;
        if (sectionId === "candidate_signals" && !config.includeCandidates)
            continue;
