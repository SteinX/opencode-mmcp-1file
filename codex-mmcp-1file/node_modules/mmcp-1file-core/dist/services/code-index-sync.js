    ...
    try {
        ...
        if (version === 2 || (version !== 3 && rawWorkspaces)) {
            ...
            for (const [key, value] of Object.entries(rawWorkspaces)) {
                if (!isSyncMetadata(value))
                    continue;
                workspaces[key] = value;
            }
            const migratedState = { version: 3, workspaces };
            writeSyncState(config, migratedState);
            return migratedState;
        }
        // v3: parse directly
...
function writeSyncState(config, state) {
    const statePath = getIndexStatePath(config);
    if (!statePath)
        return;
...
function readWorkspaceSyncMetadata(config, workspaceDir) {
    const state = readSyncState(config);
    if (!state)
        return null;
    return state.workspaces[getWorkspaceStateKey(workspaceDir)] ?? null;
}
function writeWorkspaceSyncMetadata(config, workspaceDir, metadata) {
    const state = readSyncState(config) ?? { version: 3, workspaces: {} };
    state.workspaces[getWorkspaceStateKey(workspaceDir)] = metadata;
    writeSyncState(config, state);
}
function hasIgnoredDirectorySegment(path) {
    return path.split("/").some((segment) => IGNORED_DIRECTORIES.has(segment));
}
function shouldTrackPathForCodeIndex(path) {
    if (hasIgnoredDirectorySegment(path))
        return false;
    const baseName = path.split("/").pop() || path;
    if (TRACKED_FILENAMES.has(baseName))
        return true;
...
function walkTrackedFiles(rootDir, currentDir, output) {
    const entries = readdirSync(currentDir, { withFileTypes: true });
    for (const entry of entries) {
        const fullPath = join(currentDir, entry.name);
        const relPath = relative(rootDir, fullPath);
        if (entry.isDirectory()) {
            if (hasIgnoredDirectorySegment(relPath))
                continue;
        ...
        if (!entry.isFile())
            continue;
        if (!shouldTrackPathForCodeIndex(relPath))
            continue;
        output.push(fullPath);
    }
}
export function computeWorkspaceFingerprint(workspaceDir) {
    try {
...
export function decideIndexSyncAction(status, config) {
    ...
    if (reasonCode === "active_index_running" ||
        state === "queued" ||
        state === "running") {
        return "wait";
    }
    ...
    if (reasonCode === "lost_one_shot_indexing_task_after_restart" ||
...
async function pollUntilComplete(config, workspaceDir) {
    const resumeConfig = config.codeIndexSync.resume;
...
async function runReindex(config, workspaceDir, reason) {
    const workspaceKey = getWorkspaceKey(config, workspaceDir);
    if (inFlightRuns.has(workspaceKey))
        return;
    ...
    try {
        ...
        if (metadata?.fingerprint === fingerprint && (metadata?.filterSignature ?? "{}") === currentFilterSig)
            return;
...
export async function ensureCodeIndexFresh(config, workspaceDir, reason = "session.idle") {
    if (!config.codeIndexSync.enabled || !config.codeIndexSync.autoRefresh || !workspaceDir)
        return;
    ...
    if (metadata?.fingerprint === fingerprint && savedFilterSig === currentFilterSig)
        return;
    ...
    const workspaceKey = getWorkspaceKey(config, workspaceDir);
    if (scheduledRuns.has(workspaceKey) || inFlightRuns.has(workspaceKey))
        return;
...
export function resetCodeIndexSyncState() {
