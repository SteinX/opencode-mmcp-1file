...
const lastCapturedMessageId = new Map();
export function getLastCapturedId(sessionID) {
    return lastCapturedMessageId.get(sessionID);
}
export async function performAutoCapture(config, sessionID, messages, callLLM) {
    ...
    if (uncapturedMessages.length < 2)
        return false;
...
function buildSummarizationPrompt(userText, assistantText, toolSummary, language) {
